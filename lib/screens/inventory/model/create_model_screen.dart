...
class CreateModelScreen extends StatefulWidget {
  ...
  const CreateModelScreen({super.key, required this.onRouteSelected});
...
class _CreateModelScreenState extends State<CreateModelScreen> {
  ...
  Future<void> _handleSave() async {
    ...
    try {
      await ProductModelService.instance.create(model);
      if (mounted) {
        setState(() => _isSaving = false);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Model created successfully!'),
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to save model: $error'),
            backgroundColor: AppColors.danger,
          ),
        );
      }
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Add a new model to organize your inventory products.',
